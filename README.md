...
The CPU default is H.265, CRF 18, `medium`, grayscale, closed GOP, a 250-frame keyframe interval, and no audio. Expected CPU performance is approximately 20× compression, with encoding time approximately 1.2× the video duration. `--gpu` selects NVIDIA H.265 NVENC with CQ 28 and preset `p5`; its default estimates are 10× compression and 0.25× video duration. Actual time and size depend on image content and hardware.

SEQ already stores every frame as a JPEG-compressed image. Decoding those JPEG frames and applying a lossless video codec cannot recover information discarded by JPEG and, on the tested data, produces files larger than the original SEQ. `seqcomp` therefore provides only H.264/H.265 CRF/CQ lossy compression, intended to remove additional temporal redundancy between adjacent behavioral-video frames.
...
For an existing source environment, use `conda env update --name seqcomp --file environment.yml`. `seqcomp` only uses the FFmpeg installed inside the active environment. CPU mode checks the selected libx264/libx265 encoder; GPU mode additionally requires an NVENC-capable NVIDIA GPU and a compatible, current NVIDIA driver. No CUDA Toolkit Python package is required.
...
**`seqcomp compress <file-or-folder> [--dest DIR] [--codec {h264,h265}] [--crf N] [--preset NAME] [--keyint N] [--gpu] [--cq N] [--gpu-preset {p1..p7}] [--gpu-device INDEX] [--delete] [--dry-run] [--force] [--yes] [--quiet]`**
...
- CPU defaults: `--codec h265 --crf 18 --preset medium --keyint 250`. CPU and GPU both use IDX-guided JPEG payload extraction with FFmpeg MJPEG decoding; verification and SHA-256 calculation are always enabled.
...
- GPU output is HEVC Main, 8-bit 4:2:0, full-range `bt470bg`, and contains no audio. Main/4:2:0 requires even width and height; odd-sized recordings are rejected without padding or resizing and can still be compressed with the CPU default.
...
- CPU estimates default to `--ratio 20` and `--time-ratio 1.2`; `--gpu` uses 10 and 0.25 and first verifies NVENC support. Explicit ratio options override either estimate without changing compression settings.
...
- **No byte-exact restore**: H.264/H.265 CRF or CQ output is lossy, so this project does not provide a command that claims to restore the original `.seq` file. Keep `X.mkv + X.timestamps.npy + X.manifest.json` together.
